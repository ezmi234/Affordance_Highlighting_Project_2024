...
class MinkowskiFCNN(ME.MinkowskiNetwork):
    ...
    def forward(self, xyz, features, device="cuda", quantization_size=0.05):
        ...
        # Avoid global pooling here, preserve the per-point features
    ...
    #     y4 = self.pool(y)
    #
    #     # Skip the global pooling, preserve the per-point features
    #     x1 = y1.slice(x)
    #     x2 = y2.slice(x)
    #     x3 = y3.slice(x)
    #     x4 = y4.slice(x)
    #
    #     # Concatenate all levels of the features
    #     x = ME.cat(x1, x2, x3, x4)
    #
    #     # Avoid global pooling here, directly return the features
    #     return x.F
